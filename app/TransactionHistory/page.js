...
function ContactList() {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = transactionHistory.filter(
      (transaction) =>
        transaction.fromWalboId.toLowerCase().includes(query) ||
        transaction.toWalboId.toLowerCase().includes(query) ||
  ...
  const fetchTransactionHistory = async () => {
    try {
      ...
      if (data.exists) {
        ...
        setFilteredTransactionHistory(data.transactionHistory || []);
      } else {
    ...
    <>
      ...
      <div className="input-group mt-3 container">
        <input
          ...
          name="search"
          id="search"
          placeholder="Search a Transaction by Name or Walbo ID"
          value={searchQuery}
      ...
      <div className="container">
        {info ? (
          ...
          <div className="table-responsive">
            <table
              className={`table table-hover container text-end ${
                isDark ? "table-dark" : ""
              }`}
            >
              <thead>
